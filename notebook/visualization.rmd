...
SARS_data = SARS_data %>% select("Date", "Still infected") %>% rename(still_infected = `Still infected`) %>% mutate(date=as_date(Date, format="%b %d %Y") ) %>% select("date", "still_infected")
...
df_sum_stats_sars = left_join(art_sum_stats_sars, SARS_data) %>% select("date", "article_id", "still_infected") %>% filter(date <= last_date) %>%
    
		ggplot(aes(x=date)) +

		geom_line(aes(y=article_id, color="Number of relevant articles"), size=0.5) +
		geom_line(aes(y=still_infected/b+a, color="Still infected cases (official)"), size=0.5) + 
    
        scale_linetype_manual(values=c("solid", "solid")) +

		scale_x_date(			
            breaks = seq.Date(as.Date("2003-04-03"), as.Date("2003-07-05"), by="1 month"), 
            labels = date_format("%b %Y"), 
            limits= c(as.Date("2003-04-03")-6, as.Date("2003-07-05")+6),
            expand = c(0, 0)
			) + 
		xlab("") +

		scale_y_continuous(name = "Number of articles", breaks = seq(0,70,20), limits=c(0,70),
			sec.axis = sec_axis(~(.-a)*b, name = "Still infected", breaks = seq(0,3000,1000), labels = scales::comma)) + 
    	...
    	ww = 6.75
    	ggsave("../results/sum-stats-sars.png", plot = df_sum_stats_sars, width=ww, height=(2/3)*ww)

a = 0
b = 850

df_sum_stats_covid = left_join(art_sum_stats_covid, df_covid) %>% select("date", "article_id", "still_infected") %>% filter(date <= last_date) %>%
		...
		scale_y_continuous(name = "Number of articles", breaks = seq(0,70,20), limits=c(0,70),
			sec.axis = sec_axis(~(.-a)*b, name = "Still infected", breaks = seq(0,60000,20000), labels = scales::comma)) + 
